...
Use this skill when the declarative manifest has already selected `dynamic_crawl` and an operator needs the human-readable intent behind the discovery workflow.

## Intent

- Use Playwright-based crawling to map in-scope pages and forms.
...
- verify prerequisites before trusting output: runtime `playwright`; related tools `tech_stack_fingerprint`
