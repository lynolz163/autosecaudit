...
Use this skill when the declarative manifest has already selected `cors_misconfiguration` and an operator needs the human-readable intent behind the testing workflow.

## Intent

- Probe origins for arbitrary Origin reflection and credentialed wildcard CORS behavior.
- evaluate `origin_url` targets resolved from `origins` within the declared planner phases: deep_testing
- keep analyst decisions aligned with the YAML manifest for triggers, risk posture, dependencies, and follow-up chaining

## Boundaries

- stay inside the declared risk posture: `medium` with planner cost `5`
