...
Use this skill when the declarative manifest has already selected `postgres_handshake_probe` and an operator needs the human-readable intent behind the validation workflow.

## Intent

- Collect PostgreSQL startup metadata and TLS support without authenticating.
...
- verify prerequisites before trusting output: runtime `no special runtime declared`; related tools `nmap_scan, service_banner_probe`
