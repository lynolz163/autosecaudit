...
Use this skill when the declarative manifest has already selected `cookie_security_audit` and an operator needs the human-readable intent behind the testing workflow.

## Intent

- Audit Secure, HttpOnly, and SameSite attributes on set-cookie headers.
- evaluate `origin_url` targets resolved from `origins` within the declared planner phases: deep_testing, verification
- keep analyst decisions aligned with the YAML manifest for triggers, risk posture, dependencies, and follow-up chaining

## Boundaries

- stay inside the declared risk posture: `low` with planner cost `3`
...
- downstream investigation candidates currently declared by the manifest: `csp_evaluator`
