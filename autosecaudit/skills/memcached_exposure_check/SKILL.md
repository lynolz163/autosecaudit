# Memcached Exposure Check

Use this skill when the declarative manifest has already selected `memcached_exposure_check` and an operator needs the human-readable intent behind the validation workflow.

## Intent

- Safely verify whether memcached answers version and stats probes without authentication.
- evaluate `service_port` targets resolved from `surface` within the declared planner phases: passive_recon, verification
...
- verify prerequisites before trusting output: runtime `socket`; related tools `nmap_scan, service_banner_probe`
...
- downstream investigation candidates currently declared by the manifest: `cve_lookup, rag_intel_lookup`
...
- compare the output with nearby evidence from `nmap_scan, service_banner_probe` or equivalent discovery artifacts
