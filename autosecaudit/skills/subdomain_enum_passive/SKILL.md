...
Use this skill when the declarative manifest has already selected `subdomain_enum_passive` and an operator needs the human-readable intent behind the recon workflow.

## Intent

- Enumerate likely subdomains through passive certificate transparency sources.
- evaluate `domain` targets resolved from `scope` within the declared planner phases: passive_recon
