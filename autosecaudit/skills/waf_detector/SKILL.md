...
Use this skill when the declarative manifest has already selected `waf_detector` and an operator needs the human-readable intent behind the recon workflow.

## Intent

- Identify common CDN and WAF protection layers from passive response markers.
...
- downstream investigation candidates currently declared by the manifest: `cors_misconfiguration, http_security_headers, passive_config_audit, security_txt_check, ssl_expiry_check`
