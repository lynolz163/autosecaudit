...
Use this skill when the declarative manifest has already selected `dns_zone_audit` and an operator needs the human-readable intent behind the recon workflow.

## Intent

- Resolve NS/MX/TXT/SOA records and test whether AXFR is exposed.
- evaluate `domain` targets resolved from `scope` within the declared planner phases: passive_recon, verification
...
- downstream investigation candidates currently declared by the manifest: `reverse_dns_probe, subdomain_enum_passive`
...
- confirm the current target still matches `scope` -> `domain` before reviewing results
