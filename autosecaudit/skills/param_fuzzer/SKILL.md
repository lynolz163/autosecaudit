...
Use this skill when the declarative manifest has already selected `param_fuzzer` and an operator needs the human-readable intent behind the testing workflow.

## Intent

- Run bounded benign parameter probes against parameterized endpoints.
- evaluate `parameterized_endpoint` targets resolved from `surface.endpoint_params` within the declared planner phases: deep_testing
- keep analyst decisions aligned with the YAML manifest for triggers, risk posture, dependencies, and follow-up chaining

## Boundaries

- stay inside the declared risk posture: `medium` with planner cost `6`
