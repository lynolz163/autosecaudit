...
Use this skill when the declarative manifest has already selected `security_txt_check` and an operator needs the human-readable intent behind the recon workflow.

## Intent

- Check /.well-known/security.txt for presence and baseline completeness.
...
- stay inside the declared risk posture: `safe` with planner cost `1`
